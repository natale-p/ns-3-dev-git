              ...
              if (item->m_sacked)
                {
                  ...
                  NS_ASSERT (beginOfCurrentPacket + current->GetSize () <= m_highestSack.second);
                }
              else
                {
                  item->m_sacked = true;
  ...
  for (it = segment; it != m_highestSack.first; ++it)
    {
      ...
      if (item->m_sacked)
        {
          NS_LOG_DEBUG ("Segment [" << beginOfCurrentPacket << ", " <<
                        beginOfCurrentPacket+item->m_packet->GetSize () <<
                        "] found to be SACKed, increasing counter and bytes by " <<
                        current->GetSize ());
          ++count;
