  ...
  : m_semName (semName)
{
  ...
  SpinExec ("PRAGMA synchronous = OFF");
...
bool SQLiteOutput::CheckError (sqlite3 *db, int rc, const std::string &cmd,
  ...
  if (rc != SQLITE_OK && rc != SQLITE_DONE)
    {
      ...
      if (hardExit)
        ...
        {
          NS_LOG_UNCOND (sqlite3_errmsg (db));
        }
      return true;
    }
  return false;
