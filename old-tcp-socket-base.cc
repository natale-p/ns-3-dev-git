...
Ns3TcpSocketImpl::Ns3TcpSocketImpl (void)
  : TcpSocket (),
    m_retxEvent (),
    m_lastAckEvent (),
    m_delAckEvent (),
    ...
    m_isFirstPartialAck (true)
{
  ...
  ok = m_tcb->TraceConnectWithoutContext ("CongestionWindow",
                                          MakeCallback (&Ns3TcpSocketImpl::UpdateCwnd, this));
  NS_ASSERT (ok == true);

  ok = m_tcb->TraceConnectWithoutContext ("SlowStartThreshold",
                                          MakeCallback (&Ns3TcpSocketImpl::UpdateSsThresh, this));
  NS_ASSERT (ok == true);

  ok = m_tcb->TraceConnectWithoutContext ("CongState",
                                          MakeCallback (&Ns3TcpSocketImpl::UpdateCongState, this));
  NS_ASSERT (ok == true);

  ok = m_tcb->TraceConnectWithoutContext ("NextTxSequence",
                                          MakeCallback (&Ns3TcpSocketImpl::UpdateNextTxSequence, this));
  NS_ASSERT (ok == true);

  ok = m_tcb->TraceConnectWithoutContext ("HighestSequence",
                                          MakeCallback (&Ns3TcpSocketImpl::UpdateHighTxMark, this));
  NS_ASSERT (ok == true);
}

Ns3TcpSocketImpl::Ns3TcpSocketImpl (const Ns3TcpSocketImpl& sock)
  : TcpSocket (sock),
  ...
  if (m_endPoint != 0)
    {
       ...
       * DestroyCallback is set to Ns3TcpSocketImpl::Destroy. If we called
       * m_tcp->DeAllocate, it wil destroy its Ipv4EndpointDemux::DeAllocate,
       * which in turn destroys my m_endPoint, and in turn invokes
       * Ns3TcpSocketImpl::Destroy to nullify m_node, m_endPoint, and m_tcp.
       */
...
{
  NS_LOG_FUNCTION (this);
  NS_ABORT_MSG_IF (flags, "use of flags is not supported in Ns3TcpSocketImpl::Recv()");
...
/* Tell if a sequence number range is out side the range that my rx buffer can
    accpet */
bool
Ns3TcpSocketImpl::OutOfRange (SequenceNumber32 head, SequenceNumber32 tail) const
{
...
void
Ns3TcpSocketImpl::DoForwardUp (Ptr<Packet> packet, const Address &fromAddress,
                            const Address &toAddress)
{
  ...
  if (m_rWnd.Get () == 0 && m_persistEvent.IsExpired ())
    { // Zero window: Enter persist state to send 1 byte to probe
      ...
      m_persistEvent = Simulator::Schedule (m_persistTimeout, &Ns3TcpSocketImpl::PersistTimeout, this);
...
void
Ns3TcpSocketImpl::ProcessEstablished (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
...
bool
Ns3TcpSocketImpl::IsTcpOptionEnabled (uint8_t kind) const
{
...
void
Ns3TcpSocketImpl::ReadOptions (const TcpHeader &tcpHeader, bool &scoreboardUpdated)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::LimitedTransmit ()
{
  NS_LOG_FUNCTION (this);
...
void
Ns3TcpSocketImpl::EnterRecovery ()
{
  NS_LOG_FUNCTION (this);
...
void
Ns3TcpSocketImpl::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::ProcessAck (const SequenceNumber32 &ackNumber, bool scoreboardUpdated)
{
...
void
Ns3TcpSocketImpl::ProcessListen (Ptr<Packet> packet, const TcpHeader& tcpHeader,
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  Ptr<Ns3TcpSocketImpl> newSock = Fork ();
  NS_LOG_LOGIC ("Cloned a Ns3TcpSocketImpl " << newSock);
  Simulator::ScheduleNow (&Ns3TcpSocketImpl::CompleteFork, newSock,
                          packet, tcpHeader, fromAddress, toAddress);
}

/* Received a packet upon SYN_SENT */
void
Ns3TcpSocketImpl::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
  ...
  if (tcpflags == 0)
    { // Bare data, accept it and move to ESTABLISHED state. This is not a normal behaviour. Remove this?
      ...
      ReceivedData (packet, tcpHeader);
      Simulator::ScheduleNow (&Ns3TcpSocketImpl::ConnectionSucceeded, this);
    }
...
void
Ns3TcpSocketImpl::ProcessSynRcvd (Ptr<Packet> packet, const TcpHeader& tcpHeader,
                               const Address& fromAddress, const Address& toAddress)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::ProcessWait (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::ProcessClosing (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::ProcessLastAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::PeerClose (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
...
void
Ns3TcpSocketImpl::DoPeerClose (void)
{
  ...
  if (m_state == LAST_ACK)
    {
      NS_LOG_LOGIC ("Ns3TcpSocketImpl " << this << " scheduling LATO1");
      Time lastRto = m_rtt->GetEstimate () + Max (m_clockGranularity, m_rtt->GetVariation () * 4);
      m_lastAckEvent = Simulator::Schedule (lastRto, &Ns3TcpSocketImpl::LastAckTimeout, this);
    }
}
...
Ns3TcpSocketImpl::SendEmptyPacket (uint8_t flags)
{
  ...
  if (m_retxEvent.IsExpired () && (hasSyn || hasFin) && !isAck )
    { // Retransmit SYN / SYN+ACK / FIN / FIN+ACK to guard against lost
      ...
      m_retxEvent = Simulator::Schedule (m_rto, &Ns3TcpSocketImpl::SendEmptyPacket, this, flags);
    }
}

/* This function closes the endpoint completely. Called upon RST_TX action. */
void
Ns3TcpSocketImpl::SendRST (void)
{
  NS_LOG_FUNCTION (this);
...
/* This function is called only if a SYN received in LISTEN state. After
   Ns3TcpSocketImpl cloned, allocate a new end point to handle the incoming
   connection and send a SYN+ACK to complete the handshake. */
void
Ns3TcpSocketImpl::CompleteFork (Ptr<Packet> p, const TcpHeader& h,
                             const Address& fromAddress, const Address& toAddress)
{
...
void
Ns3TcpSocketImpl::ConnectionSucceeded ()
...
Ns3TcpSocketImpl::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (m_retxEvent.IsExpired ())
    {
      ...
      NS_LOG_LOGIC (this << " SendDataPacket Schedule ReTxTimeout at time " <<
                    Simulator::Now ().GetSeconds () << " to expire at time " <<
                    (Simulator::Now () + m_rto.Get ()).GetSeconds () );
      m_retxEvent = Simulator::Schedule (m_rto, &Ns3TcpSocketImpl::ReTxTimeout, this);
    }
  ...
  if (seq + sz > m_tcb->m_highTxMark)
    {
      Simulator::ScheduleNow (&Ns3TcpSocketImpl::NotifyDataSent, this,
...
Ns3TcpSocketImpl::SendPendingData (bool withAck)
{
  ...
  if (m_endPoint == 0 && m_endPoint6 == 0)
    {
      NS_LOG_INFO ("Ns3TcpSocketImpl::SendPendingData: No endpoint; m_shutdownSend=" << m_shutdownSend);
  ...
  if (m_bytesInFlight != bytesInFlight)
    {
      ...
      const_cast<Ns3TcpSocketImpl*> (this)->m_bytesInFlight = bytesInFlight;
    }
...
void
Ns3TcpSocketImpl::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  NS_LOG_FUNCTION (this << tcpHeader);
      ...
      if (++m_delAckCount >= m_delAckMaxCount)
      ...
      else if (m_delAckEvent.IsExpired ())
        {
          m_delAckEvent = Simulator::Schedule (m_delAckTimeout,
                                               &Ns3TcpSocketImpl::DelAckTimeout, this);
 ...
 */
void
Ns3TcpSocketImpl::EstimateRtt (const TcpHeader& tcpHeader)
{
...
void
Ns3TcpSocketImpl::NewAck (SequenceNumber32 const& ack, bool resetRTO)
{
...
void
Ns3TcpSocketImpl::DelAckTimeout (void)
{
  m_delAckCount = 0;
  SendEmptyPacket (TcpHeader::ACK);
}
...
void
Ns3TcpSocketImpl::LastAckTimeout (void)
{
  NS_LOG_FUNCTION (this);
...
void
Ns3TcpSocketImpl::PersistTimeout ()
{
...
Ns3TcpSocketImpl::TimeWait ()
{
  ...
  m_timewaitEvent = Simulator::Schedule (Seconds (2 * m_msl),
                                         &Ns3TcpSocketImpl::CloseAndNotify, this);
}
...
void Ns3TcpSocketImpl::UpdateWindowSize (const TcpHeader &header)
{
  NS_LOG_FUNCTION (this << header);
...
void
Ns3TcpSocketImpl::UpdateCwnd (uint32_t oldValue, uint32_t newValue)
{
  m_cWndTrace (oldValue, newValue);
}

void
Ns3TcpSocketImpl::UpdateSsThresh (uint32_t oldValue, uint32_t newValue)
{
  m_ssThTrace (oldValue, newValue);
}

void
Ns3TcpSocketImpl::UpdateCongState (TcpSocketState::TcpCongState_t oldValue,
...
void
Ns3TcpSocketImpl::UpdateNextTxSequence (SequenceNumber32 oldValue,
...
void
Ns3TcpSocketImpl::UpdateHighTxMark (SequenceNumber32 oldValue, SequenceNumber32 newValue)
{
  m_highTxMarkTrace (oldValue, newValue);
}

Ptr<Ns3TcpSocketImpl>
Ns3TcpSocketImpl::Fork (void)
{
  return CopyObject<Ns3TcpSocketImpl> (this);
}

uint32_t
Ns3TcpSocketImpl::SafeSubtraction (uint32_t a, uint32_t b)
{
  if (a > b)
    {
      return a-b;
